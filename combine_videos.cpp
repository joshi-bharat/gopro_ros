...
ros::Time saveBag(rosbag::Bag &bag, const std::string &video_file, std::string &img_topic,
                  ros::Time start_time, double duration, double scaling_factor, bool compressed)
{
    ...
    if (videoStreamIndex == -1)
        ROS_FATAL_STREAM("Didn't find a video stream");
    ...
    if (pFrameRGB == nullptr)
        ROS_FATAL_STREAM("Cannot allocate RGB Frame");
    ...
    while (av_read_frame(pFormatContext, &packet) >= 0)
    {
        ...
        if (packet.stream_index == videoStreamIndex)
        {
            ...
            if (frameFinished)
            {
                ...
                if (compressed)
                {
                    sensor_msgs::CompressedImagePtr img_msg = cv_bridge::CvImage(header, "bgr8", img).toCompressedImageMsg();
