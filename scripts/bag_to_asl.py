...
class dump_stamps:
    def __init__(self, root_folder):
        ...
        rospy.Subscriber('/gopro/image/compressed', CompressedImage, self.compressed_image_sub)
    ...
    def compressed_image_sub(self, img_msg):
        ...
        cv2.imwrite(os.path.join(self.img_data_folder,  '{}.png'.format(str(img_msg.header.stamp))), image_np)
        self.img_file.write('{},{}\n'.format(str(img_msg.header.stamp), '{}.png'.format(str(img_msg.header.stamp))))

    def image_sub(self, img_msg):
        cv_image = self.bridge.imgmsg_to_cv2(img_msg, desired_encoding='passthrough')
        cv2.imwrite(os.path.join(self.img_data_folder, '{}.png'.format(str(img_msg.header.stamp))), cv_image)
        self.img_file.write('{},{}\n'.format(str(img_msg.header.stamp), '{}.png'.format(str(img_msg.header.stamp))))
